...
class AboutController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(isset($page) && is_numeric($page) && $page>0) {
            ...
            $abouts = $query->skip($records_to_skip)
            ->take($records_to_show)
            ->get();

            return view('abouts.index', compact('abouts','num_of_pages', 'page'));

            
        } else {
            return redirect()->route('abouts.index', ['page'=>1]);
        }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            // Upload Main Image
            $imgName = $this->uploadFile($request, 'main_image', 'abouts_imgs', 'required|mimes:jpeg,jpg,png,webp|max:1024');
            $inputs['main_image'] = $imgName;

            // Store
            $insert = About::create($inputs);
    ...
    public function show($id) {
        ...
        return view('abouts.form', compact('about'));
    }
    ...
    public function destroy(Request $request)
    {
        ...
        $about = About::where('id', $request->record_id)->first();

        if($about) {

            $about->fill(['deleted' => 1]);
    ...
    public function requestValidations($record_id = null) {
        ...
        $slugUniqueRule = Rule::unique('abouts', 'slug')->ignore($record_id);
