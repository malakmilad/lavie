...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">Dashboard</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ reviews</span>
			</div>
		</div>
	</div>
	<!-- breadcrumb -->
							...
							<table class="table text-md-nowrap" id="example1">
								<thead>
									<tr>
										...
										<th class="all border-bottom-0">media</th>
										<th class="all border-bottom-0">media type</th>
								...
								<tbody>
									...
									<tr>
										<td>{{$review->id}}</td>
										<td>
											<a href="{{config('app.uploads')}}{{$review->media_src}}" target="_blank">Preview <i class="fa fa-eye"></i></a>
										</td>
										<td>{{$review->media_type}}</td>
										<td>{{date('d M Y', strtotime($review->created_at))}}</td>
										<td>
											<a href="#" class="deleteRow mx-1" data-id="{{$review->id}}"><i class="fa-solid fa-trash text-danger"></i></a>
											<a href="reviews/{{$review->id}}" class="mx-1"><i class="fa-solid fa-pen-to-square"></i></a>
										</td>
									</tr>
									@endforeach
								</tbody>
...
<script>
	...
	function deleteRecord(record_id) {
		...
		var ajaxPromise = ajaxRequest('POST', "{{url('admin/delete/review')}}", {record_id: record_id});
