...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">Dashboard</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ videos</span>
			</div>
		</div>
	</div>
	<!-- breadcrumb -->
							...
							<table class="table text-md-nowrap" id="example1">
								...
								<tbody>
									...
									<tr>
										<td>{{$video->id}}</td>
										<td>
											<a href="{{$video->media_src}}" target="_blank">{{$video->media_src}}</a>
										</td>
										<td>{{date('d M Y', strtotime($video->created_at))}}</td>
										<td>
											<a href="#" class="deleteRow mx-1" data-id="{{$video->id}}"><i class="fa-solid fa-trash text-danger"></i></a>
											<a href="videos/{{$video->id}}" class="mx-1"><i class="fa-solid fa-pen-to-square"></i></a>
										</td>
									</tr>
									@endforeach
								</tbody>
...
<script>
	...
	function deleteRecord(record_id) {
		...
		var ajaxPromise = ajaxRequest('POST', "{{url('admin/delete/video')}}", {record_id: record_id});
